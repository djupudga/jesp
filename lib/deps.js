...
/**
 * Returns classpath for all installed dependencies.
 * @param {string|undefined} Append this to classpath
 * @return {string} Deps classpath
 */
export function depsClasspath(cp) {
  ...
  if (cp) {
    classpath = classpath ? `${classpath}:${cp}` : cp
  }
  return cp ? `-cp ${classpath}` : ""
}
/**
